...
public class GameStartingEvent extends CoreAbstractEvent {
    ...
    public void tickCountdownSecond(int countdown) {
        switch (countdown) {
            case 15, 10, 5, 4, 3, 2, 1 -> {
                GameCore.INSTANCE.sendAllMessage(MutableText.of(new LiteralTextContent("§e游戏将在 §c" + countdown + " §e秒后开始")));
                GameCore.INSTANCE.allPlayerHandler(playerEntity -> {
                            ...
                            GameCore.INSTANCE.sendTitle(playerEntity, MutableText.of(new LiteralTextContent("§c" + countdown)), MutableText.of(new LiteralTextContent("§e游戏即将开始")), 0, 10, 20);
                        }
                );
            }
        }
    ...
    public void callEvent() {
        ...
        serverWorld.getWorldBorder().setSize(650);
